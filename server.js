...
//  res.sendFile(__dirname + '/public/index.html');
  ...
  collection.find({}).toArray(function(err, docs) {
    ...
    fs.readFile(path.join(__dirname, "public","index.html"), 'utf8', (err2, data2)=>{
      if (err2) {
...
app.get('/getAllProducts', (req, res) => {
  ...
  collection.find({}).toArray(function(err, docs) {
    assert.equal(err, null);
    console.log("Found the following records in /getAllTasks");
    //console.log(docs);
...
app.get('/removeProduct/:id', (req, res) => {
  ...
  const collection = db.collection(dbColl);
  // Find some documents
  collection.removeOne( { "id" : id } , function(err, result) {
    assert.equal(err, null);
    //assert.equal(3, result.result.n);
    //assert.equal(3, result.ops.length);
    console.log("Inserted 1 documents into the collection");
